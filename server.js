...
app.post('/Customer/Reset-Orders', (req, res) => {
    database.all('SELECT * FROM Orders', (err, result) => {
        ...
        // console.log(result);
        result.forEach((row) => {
            database.get('SELECT * FROM Stock WHERE product_name = ?', [row.product_name], (err, product) => {
                ...
                intQuantity -= intOrder;
                database.run('UPDATE Stock SET quantity = ? WHERE product_name = ?', [intQuantity, row.product_name], (err) => {
                    if (err) {
                        throw err;
                    }
                });
    ...
    database.get('SELECT * FROM Stock WHERE product_name = ?', [name], (err, result) => {
        ...
        database.get('SELECT * FROM Orders WHERE product_name = ?', [name], (err, row) => {
            ...
            if (err) {
                ...
                database.run('UPDATE Orders SET quantity = ? AND price = ? WHERE product_name = ?', [orderQuantity, orderPrice, row.product_name], (err) => {
                    if (err) {
                        throw err;
                    }
                    res.send('Order-Added');
